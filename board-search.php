...
$CanViewUnindexed = isset(Vi::$mod["type"]) && Vi::$mod["type"] <= GlobalVolunteer;
...
if (isset($_GET['lang']) && $_GET['lang'] != "" && isset(Vi::$config['languages'][$_GET['lang']])) {
...
foreach ($boards as $board) {
	...
	// get board locale
	$boardLang = languageName(BoardLocale($board['uri']));

	// Check against our config search options.
	if($search['lang'] !== false && $search['lang'] != $boardLang['short']) {
		continue;
	}
	$board['locale'] = $boardLang['full'];

...
class Image {
	...
	public function __construct($src, $format = false, $size = false) {
		...
		if (Vi::$config['thumb_method'] == 'imagick') {
			$classname = 'ImageImagick';
		} elseif (in_array(Vi::$config['thumb_method'], array('convert', 'convert+gifsicle', 'gm', 'gm+gifsicle'))) {
	...
	public function resize($extension, $max_width, $max_height) {
		if (Vi::$config['thumb_method'] == 'imagick') {
			$classname = 'ImageImagick';
		} elseif (Vi::$config['thumb_method'] == 'convert') {
			$classname = 'ImageConvert';
		} elseif (Vi::$config['thumb_method'] == 'convert+gifsicle') {
			$classname = 'ImageConvert';
			$gifsicle  = true;
		} elseif (Vi::$config['thumb_method'] == 'gm') {
			$classname = 'ImageConvert';
			$gm        = true;
		} elseif (Vi::$config['thumb_method'] == 'gm+gifsicle') {
			$classname = 'ImageConvert';
			$gm        = true;
			$gifsicle  = true;
		} else {
...
class ImageImagick extends ImageBase {
	...
	public function from() {
		try {
			...
			if(strtolower($this->image->format) == 'gif') {
	...
	public function to($src) {
		...
		if (preg_match('/\.gif$/i', $src)) {
			$this->image->writeImages(getcwd() . DIRECTORY_SEPARATOR . $src, true);
		} else {
			$this->image->writeImage(getcwd() . DIRECTORY_SEPARATOR . $src);
		}
	}
	...
	public function resize() {
		if ($this->format == 'gif' && (Vi::$config['thumb_ext'] == 'gif' || Vi::$config['thumb_ext'] == '' || Vi::$config['gif_preview_animate'])) {
			$this->image = new Imagick();
			$this->image->setFormat('gif');

			$keep_frames = array();
			for ($i = 0; $i < $this->original->getNumberImages(); $i += floor($this->original->getNumberImages() / Vi::$config['thumb_keep_animation_frames'])) {
				$keep_frames[] = $i;
			}

			$i     = 0;
			$delay = 0;
			$frames = $this->original->coalesceImages();
			foreach ($frames as $frame) {
				$delay += $frame->getImageDelay();

				if (in_array($i, $keep_frames) || Vi::$config['gif_preview_animate']) {
...
class ImageConvert extends ImageBase {
	...
	public function init() {
		if (Vi::$config['thumb_method'] == 'gm' || Vi::$config['thumb_method'] == 'gm+gifsicle') {
			$this->gm = true;
		}

		if (Vi::$config['thumb_method'] == 'convert+gifsicle' || Vi::$config['thumb_method'] == 'gm+gifsicle') {
			$this->gifsicle = true;
		}
	...
	public function resize() {
		...
		$this->temp = tempnam(Vi::$config['tmp'], 'convert');

		Vi::$config['thumb_keep_animation_frames'] = (int) Vi::$config['thumb_keep_animation_frames'];

		if ($this->format == 'gif' && (Vi::$config['thumb_ext'] == 'gif' || Vi::$config['thumb_ext'] == '') && Vi::$config['thumb_keep_animation_frames'] > 1) {
			if ($this->gifsicle) {
				if (($error = shell_exec("gifsicle -w --unoptimize -O2 --resize {$this->width}x{$this->height} < " .
					escapeshellarg($this->src . '') . " \"#0-" . Vi::$config['thumb_keep_animation_frames'] . "\" -o " .
					escapeshellarg($this->temp))) || !file_exists($this->temp)) {
					$this->destroy();
					error(_('Failed to resize image!'), null, $error);
				}
			} else {
				if (Vi::$config['convert_manual_orient'] && ($this->format == 'jpg' || $this->format == 'jpeg')) {
					$convert_args = str_replace('-auto-orient', ImageConvert::jpeg_exif_orientation($this->src), Vi::$config['convert_args']);
				} elseif (Vi::$config['convert_manual_orient']) {
					$convert_args = str_replace('-auto-orient', '', Vi::$config['convert_args']);
				} else {
					$convert_args = &Vi::$config['convert_args'];
				}

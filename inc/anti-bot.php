...
class AntiBot {
	...
	public function __construct(array $salt = array()) {
		...
		shuffle(Vi::$config['spam']['hidden_input_names']);

		$input_count          = mt_rand(Vi::$config['spam']['hidden_inputs_min'], Vi::$config['spam']['hidden_inputs_max']);
		$hidden_input_names_x = 0;

		for ($x = 0; $x < $input_count; $x++) {
			if ($hidden_input_names_x === false || mt_rand(0, 2) == 0) {
				// Use an obscure name
				$name = $this->randomString(mt_rand(10, 40), false, false, Vi::$config['spam']['unicode']);
			} else {
				// Use a pre-defined confusing name
				$name = Vi::$config['spam']['hidden_input_names'][$hidden_input_names_x++];
				if ($hidden_input_names_x >= count(Vi::$config['spam']['hidden_input_names'])) {
			...
			if (mt_rand(0, 2) == 0) {
				...
				// Obscure value
				$this->inputs[$name] = $this->randomString(mt_rand(5, 100), true, true, Vi::$config['spam']['unicode']);
			}
		}
	}
...
function checkSpam(array $extra_salt = array()) {
	...
	foreach ($_POST as $name => $value) {
		if (in_array($name, Vi::$config['spam']['valid_inputs'])) {
			continue;
		}
	...
	$query->execute() or error(db_error($query));
	if ((($passed = $query->fetchColumn(0)) === false) || ($passed > Vi::$config['spam']['hidden_inputs_max_pass'])) {

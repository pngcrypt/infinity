...
function permission_to_edit_config_var($varname) {
	if (isset(Vi::$config['mod']['config'][DISABLED])) {
		foreach (Vi::$config['mod']['config'][DISABLED] as $disabled_var_name) {
			$disabled_var_name = explode('>', $disabled_var_name);
			if (count($disabled_var_name) == 1 || empty($disabled_var_name[1])) {
				$disabled_var_name = $disabled_var_name[0];
			}

			if ($varname == $disabled_var_name || is_array($varname) && $varname[0] == $disabled_var_name) {
				return false;
			}
	...
	$allow_only = false;
	foreach (Vi::$config['mod']['groups'] as $perm => $perm_name) {
		if ($perm > Vi::$mod['type']) {
			break;
		}

		$allow_only = false;
		if (isset(Vi::$config['mod']['config'][$perm]) && is_array(Vi::$config['mod']['config'][$perm])) {
			foreach (Vi::$config['mod']['config'][$perm] as $perm_var_name) {
				if ($perm_var_name == '!') {
					$allow_only = true;
					continue;
				}
...
function config_vars() {
	...
	foreach ($config_file as $line) {
		...
		if (preg_match('!^\s*// ([^$].*)$!', $line, $matches)) {
			...
			$var['default_temp'] .= "\n" . $line;
		} elseif (preg_match('!^[\s/]*Vi::\$config\[(.+?)\] = (.+?)(;( //.+)?)?$!', $line, $matches)) {
		...
		if ($var['name'] !== false) {
			...
			if (!preg_match('/^array|\[\]|function/', $var['default']) && !preg_match('/^Example: /', trim(implode(' ', $var['comment'])))) {
				...
				if ($var['type'] == 'integer' && $var['name'][0] == 'mod' &&
					(in_array($var['default'], array('JANITOR', 'MOD', 'ADMIN', 'DISABLED')) || mb_strpos($var['default'], "Vi::\$config['mod']") === 0)) {

...
class Api {
	public function __construct() {
		...
		if (isset(Vi::$config['api']['extra_fields']) && gettype(Vi::$config['api']['extra_fields']) == 'array') {
			$this->postFields = array_merge($this->postFields, Vi::$config['api']['extra_fields']);
		}
	}
	...
	private function translatePost($post, $threadsPage = false) {
		...
		if (Vi::$config['poster_ids']) {
			if ($post->thread) {
				$apiPost['id'] = poster_id($post->ip, $post->thread, Vi::$board['uri']);
			} else {
				$apiPost['id'] = poster_id($post->ip, $post->id, Vi::$board['uri']);
			}
		}
		...
		if (isset($post->body_nomarkup) && Vi::$config['country_flags']) {
	...
	public function translateThread(Thread $thread, $threadsPage = false) {
		$apiPosts = array();
	...
	public function translateCatalogPage(array $threads, $threadsPage = false) {
		$apiPage = array();
		foreach ($threads as $thread) {
	...
	public function translateCatalog($catalog, $threadsPage = false) {
		$apiCatalog = array();

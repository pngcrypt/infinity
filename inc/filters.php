	...
	public function action() {
		...
		if (isset($this->action)) {
			...
			case 'ban':
				...
				Bans::new_ban(GetIp(), $this->reason, $this->expires, $this->all_boards ? false : Vi::$board['uri'], -1, $this->post);
...
function purge_flood_table() {
	...
	// config variable Vi::$config['flood_cache'] (seconds).

	if (isset(Vi::$config['flood_cache'])) {
		$max_time = &Vi::$config['flood_cache'];
	} else {
		$max_time = 0;
		foreach (Vi::$config['filters'] as $filter) {
...
function do_filters(array $post) {
	if (!isset(Vi::$config['filters']) || empty(Vi::$config['filters'])) {
		return;
	}
	...
	foreach (Vi::$config['filters'] as $filter_array) {

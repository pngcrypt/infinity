	...
	public function build($mod = false) {
		...
		while ($post = $query->fetch()) {
			...
			if ($count < $this->settings['thread_limit']) {
				Vi::$config['uri_thumb'] = '/' . $post['board'] . '/thumb/';
				Vi::$config['uri_img']   = '/' . $post['board'] . '/src/';
				Vi::$board['dir']            = $post['board'] . '/';
				...
				$posts->bindValue(':limit', ($post['sticky'] ? Vi::$config['threads_preview_sticky'] : Vi::$config['threads_preview']), PDO::PARAM_INT);
				...
				if ($posts->rowCount() == ($post['sticky'] ? Vi::$config['threads_preview_sticky'] : Vi::$config['threads_preview'])) {
					...
					$c               = $ct->fetch();
					$thread->omitted = $c['num'] - ($post['sticky'] ? Vi::$config['threads_preview_sticky'] : Vi::$config['threads_preview']);
				...
				$body .= '<h2><a href="' . Vi::$config['root'] . $post['board'] . '">/' . $post['board'] . '/</a></h2>';
				$body .= $thread->build(true);
			} else {
				$page = 'index';
				if (floor($threads[$post['board']] / Vi::$config['threads_per_page']) > 0) {
					$page = floor($threads[$post['board']] / Vi::$config['threads_per_page']) + 1;
				}

...
if (!Vi::$config['search']['enable']) {
	die(_("Post search is disabled"));
}

$queries_per_minutes     = Vi::$config['search']['queries_per_minutes'];
$queries_per_minutes_all = Vi::$config['search']['queries_per_minutes_all'];
$search_limit            = Vi::$config['search']['search_limit'];

if (isset(Vi::$config['search']['boards'])) {
	$boards = Vi::$config['search']['boards'];
} else {
...
if (isset($_GET['search']) && !empty($_GET['search']) && isset($_GET['board']) && in_array($_GET['board'], $boards)) {
	...
	foreach ($filters as $name => $value) {
		...
		$like .= '`' . $name . '` = ' . Vi::$pdo->quote($value);
	}

	$like = str_replace('%', '%%', $like);

	$query = prepare(sprintf("SELECT * FROM ``posts_%s`` WHERE " . $like . " ORDER BY `time` DESC LIMIT :limit", Vi::$board['uri']));
	...
	if (!empty($temp)) {
		...
		sprintf(Vi::$config['board_path'], Vi::$board['uri']) . Vi::$config['file_index'] .
		'">' .
		sprintf(Vi::$config['board_abbreviation'], Vi::$board['uri']) . ' - ' . Vi::$board['title'] .

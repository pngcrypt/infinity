...
$(function() {
		...
		if (storage()[board] && storage()[board].threads)
			for (var tid in storage()[board].threads) {
				...
				if (variant == 'desktop') {
					...
					tag = $("<a href='" + modRoot + board + "/res/" + tid + ".html'><span>#" + tid + "</span><span class='cb-uri'>" + newposts + "</span>" + "<span class='cb-uri watch-remove'><i class='fa fa-minus'></i></span>");
				}
				tag.attr('data-thread', tid).addClass("cb-menuitem").appendTo(list).find(".watch-remove").click(function() {
	...
	var update_pinned = function() {
		...
		if (device_type == "desktop") bl.off().on("mouseenter", function() {
		...
		for (var i in st) {
			if (is_pinned(st[i])) {
				...
				if (st[i].watched) {
					...
					if (device_type == "desktop") link.off().mouseenter(function() {
						$('.cb-menu').remove();
						var board = $(this).attr("data-board");
						var wl = construct_watchlist_for(board, "desktop").appendTo($(this)).css("top", $(this).position().top + ($(this).css('padding-top').replace('px', '') | 0) + ($(this).css('padding-bottom').replace('px', '') | 0) + $(this).height()).css("left", $(this).position().left).css("right", "auto").css("font-style", "normal");
						if (typeof init_hover != "undefined") wl.find("a.cb-menuitem").each(init_hover);
					}).mouseleave(function() {
	...
	var check_post = function(frame, post) {
		return post.length && $(frame).scrollTop() + $(frame).height() >= post.position().top + post.height();
	}
	var check_scroll = function() {
		if (!status) return;
		var refresh = false;
		for (var bid in status) {
			if (((status[bid].new_threads && (active_page == "ukko" || active_page == "index")) || status[bid].new_threads == 1) && check_post(this, $('[data-board="' + bid + '"]#thread_' + status[bid].last_thread))) {
				var st = storage()
	...
	if (typeof add_title_collector != "undefined") add_title_collector(function() {
		if (!status) return 0;
		var sum = 0;
		for (var bid in status) {

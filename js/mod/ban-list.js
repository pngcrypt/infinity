	...
	$.getJSON(inMod ? ("?/bans.json/" + token) : token, function(t) {
		...
		lt = $("#banlist").longtable({
			...
			created: {
				...
				width: "100px",
				fmt: function(f) {
					return Vi.time.ago(f.created*1000, true, true); // in AGO form
				}
			},
			// duration?
			expires: {
				...
				fmt: function(f) {
					...
					return Vi.time.dateformat(new Date((f.expires|0) * 1000)) + ((f.expires < time()) ? "" : " <small>" + _("in ") + Vi.time.until((f.expires|0)*1000) + "</small>");
				}
			},
			username: {
				name: _("Staff"),
		...
		$("#select-all").click(function(e) {
			...
			lt.get_data().forEach(function(v) {
				if(v.access)
					selected[v.id] = $this.prop("checked");
			});
			e.stopPropagation();
		});
		var filter = function(e) {
			if ($("#only_mine").prop("checked") && ($.inArray(e.board, my_boards) === -1))
				return false;
			if ($("#only_not_expired").prop("checked") && e.expires && e.expires !== 0 && e.expires < time())
				return false;
			if ($("#search").val()) {
				...
				terms.forEach(function(t) {
					var fs = fields;
					var re = /^(mask|reason|board|staff|message):/,
						ma;
					if ((ma = t.match(re))) {
						fs = [ma[1]];
						t = t.replace(re, "");
		...
		if (device_type == 'desktop') {
			...
			$(window).on("scroll resize", function() {
				if ($(window).scrollTop() > stick_on && state) {
					...
					state = !state;
				} else if ($(window).scrollTop() < stick_on && !state) {

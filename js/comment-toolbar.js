...
/* global jQuery, _, active_page, Options */
if (active_page == 'thread' || active_page == 'index') {
	var formatText = (function($){
		...
		self.wrap = function(ref, target, option, expandedwrap) {
			...
			// set values
			res = before + settings.prefix + selected + settings.suffix + after;
			$(target).val(res);
		...
		self.build_toolbars = function(){
			if (localStorage.formatText_toolbar == 'true'){
				...
				$('[name="body"]').before('<div class="format-text">'+
					'<a href="javascript:;" onclick="formatText.toolbar_wrap(this);">' + _('Wrap') + '</a>'+
					'<select id="format-text-select">'+ options +'</select></div>'
				);
				$('body').append('<style>'+
					'.format-text {display:table; width:100%; border-spacing: 0;}'+
					'.format-text>a {width:15%; padding:0 2px; display:table-cell; text-align:center;}'+
					'.format-text>select {width:100%; display:table-cell;}'+
				'</style>');

				$('.format-text > select').on('change', function(e){
					if(!e.bubbles) return; // skip non-event calls
					$(this).parents().find('#body').focus();
				});
			}
		};
		...
		$(function() {
			...
			if (window.Options && Options.get_tab('general')) {
				s1 = '#formatText_keybinds>input';
				s2 = '#formatText_toolbar>input';
				e = 'change';
				Options.extend_tab('general',
					'<fieldset>'+
						'<legend>' + _('Formatting Options') + '</legend>'+
				...
				s1 = '#formatText_keybinds';
				s2 = '#formatText_toolbar';
				e = 'click';
			...
			if (window.Options && !Options.get_tab('formatting')) {
				...
				Options.extend_tab('formatting',
					'<style>'+
						'.format_option { margin-right:5px; overflow:initial; font-size:15px; }'+
						'.format_option[type="text"] { text-align:center; padding-bottom:2px; padding-top:2px; }'+
						'.format_option:last-child { margin-right:0; }'+
				...
				Options.extend_tab('formatting',
					...
					'<span class="format_option" style="margin-left:25px;" title="Multi-line: Allow formatted area to contain linebreaks.">ML</span>'+
					...
					'<span class="format_option" style="margin-left:15px;" title="Text injected at the start of a format area.">Prefix</span>'+
					'<span class="format_option" style="margin-left:40px;" title="Text injected at the end of a format area.">Suffix</span>'+
					'<span class="format_option" style="margin-left:25px;" title="Optional keybind value to allow keyboard shortcut access.">Key</span>'
				);
